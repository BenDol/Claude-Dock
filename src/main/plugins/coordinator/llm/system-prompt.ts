 ...
 *
 * The orchestrator runs the tool-calling loop locally for every provider —
 * the LLM picks tools by short name (`prompt_terminal`) and the orchestrator
 * dispatches each call against the dock services. There is no longer a
 * passthrough/SDK variant.
 */

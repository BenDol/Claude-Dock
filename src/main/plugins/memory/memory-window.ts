...
const STATE_KEY_PREFIX = 'memory:'

/**
 * Manages Memory Plugin BrowserWindows, one per projectDir.
 */
export class MemoryWindowManager {
  private static instance: MemoryWindowManager
  private windows = new Map<string, BrowserWindow>()
  private forceClosing = new Set<string>()

  static getInstance(): MemoryWindowManager {
    if (!MemoryWindowManager.instance) {
      MemoryWindowManager.instance = new MemoryWindowManager()
    }
    return MemoryWindowManager.instance
  }
  ...
  async open(projectDir: string): Promise<void> {
    ...
    if (existing && !existing.isDestroyed()) {
      ...
      existing.show()
      existing.focus()
      existing.webContents.send('memory:reopen')
      svc().broadcastPluginWindowState('memory', projectDir, true)
      return
    }
    ...
    const win = new BrowserWindow({
      ...
      minHeight: 500,
      frame: false,
      title: `${path.basename(projectDir)} - Memory`,
    ...
    win.webContents.on('render-process-gone', (_event, details) => {
      svc().log(`[memory] renderer gone for ${projectDir}: reason=${details.reason} exitCode=${details.exitCode}`)
      if (details.reason === 'crashed' || details.reason === 'oom' || details.reason === 'killed') {
        svc().log(`[memory] auto-reloading after crash for ${projectDir}`)
        setTimeout(() => {
          if (!win.isDestroyed()) {
            const queryParam = `?memory=true&projectDir=${encodeURIComponent(projectDir)}`
    ...
    win.on('unresponsive', () => svc().log(`[memory] window unresponsive for ${projectDir}`))
    win.on('responsive', () => svc().log(`[memory] window responsive again for ${projectDir}`))

    // Hide on close (instant reopen), destroy on force-close
    win.on('close', (event) => {
      if (this.forceClosing.has(projectDir)) return
      event.preventDefault()
      win.hide()
      svc().broadcastPluginWindowState('memory', projectDir, false)
      svc().log(`[memory] window hidden for ${projectDir}`)
    })
    ...
    win.on('closed', () => {
      ...
      svc().broadcastPluginWindowState('memory', projectDir, false)
      svc().log(`[memory] window closed for ${projectDir}`)
    })

    const queryParam = `?memory=true&projectDir=${encodeURIComponent(projectDir)}`
    await loadPluginWindow(win, svc().paths, queryParam)

    svc().log(`[memory] window opened for ${projectDir}`)
  }
